WELCOME_TEXT = "Xush kelibsiz / Добро пожаловать!"
CHOOSE_LANG = "Tilni tanlang / Выберите язык!"
BTN_LANG_UZ = "Uzbek tili"
BTN_LANG_RU = "Русский язык"
STATES = {
    "reg": 1,
    "menu": 2,
    "settings": 3
}
LANGUAGE_CODE = {
    1: "uz",
    2: "ru"
}
TEXT_LANG_WARNING = "Iltimos tillardan birini tanlang!\nПожалуйста, выберите один из языков!"
TEXT_ENTER_FIRST_NAME = {
    1: "Iltimos ismingizni kiriting!",
    2: "Пожалуйста, введите ваше имя!"
}
TEXT_ENTER_LAST_NAME = {
    1: "Iltimos familiyangizni kiriting!",
    2: "Пожалуйста, введите свою фамилию!"
}
BTN_SEND_CONTACT = {
    1: "Yuborish",
    2: "Отправить"
}
TEXT_ENTER_CONTACT = {
    1: "Iltimos telefon raqamingizni yuboring yoki <<Yuborish>> tugmasini bosing!",
    2: "Отправьте свой номер телефона или нажмите кнопку <<Отправить>>!"
}
TEXT_MAIN_MENU = {
    1: "🏠 Asosiy menyu",
    2: "🏠 Главное меню"
}
BTN_ORDER = {
    1:"🛒 Buyurtma qilish",
    2:"🛒 Заказать"
}
TEXT_ORDER = {
    1: "Kategoriylardan birini tanlang!",
    2: "Выбери одну из категорий!"
}
BTN_MY_ORDERS = {
    1:"🛍 Buyurtmalarim",
    2:"🛍 Мои заказы"
}
BTN_ABOUT_US = {
    1:"👨‍👩‍👦 Biz haqimizda",
    2:"👨‍👩‍👦 O нас"
}
BTN_KORZINKA = {
    1: "🛍 Xarid qilish",
    21: "🛍 Покупка товаров",
}
BTN_COMMENTS = {
    1:"✍️Fikr bildirish",
    2:"✍️комментарий"
}
BTN_SETTINGS = {
    1:"⚙️Sozlamalar",
    2:"⚙️Настройки"
}

########## lesson-3 ###########
TEXT_PRODUCT_PRICE = {
    1: "Narx:",
    2: "Цена:"
}
TEXT_PRODUCT_DESC = {
    1: "Tarkibi:",
    2: "Состав:"
}
###########################


# lesson-4 ##############

AT_KORZINKA = {
    1: "Savatchada",
    2: "В корзине"
}

ALL = {
    1: "Jami",
    2: "Все"
}

ZAKAZ = {
    1: "Buyurtma",
    2: "Заказ"
}

SUM = {
    1: "so'm",
    2: "сум"
}

ABOUT_COMPANY = {
    1: "Men <b>Sherzod Tilovov</b> - dasturchiman! "
       "\n\nDasturlash va texnologiyalar bilan shug'ullanaman, yangi loyihalar ustida ishlashni yoqtiraman. "
       "Maqsadim - innovatsion yechimlar yaratish va bilimlarimni boshqalar bilan bo‘lishish!\n\n"
       "<b>DOIM RIVOJLANISH UCHUN HARAKAT QILAMAN!</b>",

    2: "Я <b>Шерзод Тиловов</b> - разработчик! "
       "\n\nЗанимаюсь программированием и технологиями, люблю работать над новыми проектами. "
       "Моя цель - создавать инновационные решения и делиться знаниями с другими!\n\n"
       "<b>ВСЕГДА СТРЕМИЛСЯ К РАЗВИТИЮ!</b>"
}

NO_ZAKAZ = {
    1: "Siz hali birorta buyurtma bermadingiz!",
    2: "Вы еще не разместили ни одного заказа!"
}

SEND_LOCATION = {
    1: "📍 Lokatsiya jo'nating",
    2: "📍 Отправить местоположение"
}
#########################33